...
분산락은 데이터 일관성과 접근 제어를 보장해 안정적인 성능을 유지하면서 대규모 분산 환경에서의 자원 충돌을 방지한다.
테스트에서는 단일 서버 환경이기에 성능에 한계가 있을 수 있었지만,
분산 환경에서는 확장성과 성능이 향상되어 대규모 서비스에서 일관된 사용자 경험을 제공할 수 있다.

충전 / 결제 부분에선 우선 요청만 처리하기 위해 낙관락을 사용했지만 요청 대기 큐가 필요한 부분에선 분산락을 활용할 수 있을 것 같다.
</details>
